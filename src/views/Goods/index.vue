    ...
    <van-tabs v-model="active">
      <van-tab title="商品">
        <van-index-bar :index-list="indexList">
          <div v-for="(item,index) in foodsList" :key="index" class="foods-list-wrap">
          <van-index-anchor :index="item.name">{{item.name}}</van-index-anchor>
          <van-cell v-for="(subitem,index) in item.foods" :key="index"  :title="subitem.name" />
          <!-- <van-cell title="文本" />
          <van-cell title="文本" /> -->
          </div>
        </van-index-bar>
...
.van-index-bar__sidebar {position:fixed;left:0;right:auto;}
</style>